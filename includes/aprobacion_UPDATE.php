...
/*
  ...
  echo '<br/>'.'Nuevo nivel'.$nuevo_nivel=new_nivel_aprobacion($_POST['id_solicitud'], $_POST['id_clasificacion'], $orden_historico, $link).'<br/>'; */
    ...
    if (($_SESSION['usu_rol_aprobacion'] == 5) || ($_SESSION['usu_rol_aprobacion'] == 6)){ 
        pg_query($link, "UPDATE historico_solicitud
           SET usuario_id='" . $_SESSION['usu_id'] . "', aprobado='" . $_POST['aprobacion_id'] . "', observacions='" . $_POST['obs'] . "', req_minimos='" . $_POST['obs_requisito'] . "', resultado='" . $_POST['obs_resultado'] . "', fecha_actualizacion=now(), audit_usu_id='" . $_SESSION['usu_id'] . "', audit_ip='" . $_SERVER['REMOTE_ADDR'] . "', audit_dep_id='" . $_SESSION['usu_dependencia_id'] . "'
...
switch ($_POST['aprobacion_id']) {
    case '1':
        if (max_orden($_POST['id_solicitud'], $_POST['id_clasificacion'], $link) == max_orden_historico($_POST['id_solicitud'], $_POST['id_clasificacion'], $link)) {
           // PENDIENTE PARA COLOCAR TRUE AL FINAL DEL HISTORICO DE SOLICITUDES
